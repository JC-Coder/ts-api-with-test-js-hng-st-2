...
import { cleanupDatabase, initializeTestDataSource, testDataSource } from './test-utils';
import { User } from '../src/module/v1/user/entities/user.entity';
import { customExceptionFactory } from '../src/common/Exceptions/custom.exception';
...
describe('AuthController (e2e)', () => {
  ...
  describe('POST /auth/register', () => {
    ...
    it('should verify the default organisation is created', async () => {
      const response = await request(app.getHttpServer()).get('/organisations').set('Authorization', `Bearer ${token}`);
      ...
      expect(responseBody.organisations[0].name.split("'")[0]).toEqual(registeredUser.firstName);
      expect(responseBody.organisations[0].name).toEqual(`${registeredUser.firstName}'s Organisation`);
    });
  });
});
